...
In CVS and RCS you use $Revision: 1.7 $ writing it like so:

    $VERSION = sprintf "%d.%03d", q$Revision: 1.7 $ =~ /(\d+)/g;

On your next check in, $Revision: 1.7 $ will magically be expanded to contain
the current revision #.

    $VERSION = sprintf "%d.%03d", q$Revision: 1.7 $ =~ /(\d+)/g;

Every time the file is checked in the $Revision: 1.7 $ will be updated,
