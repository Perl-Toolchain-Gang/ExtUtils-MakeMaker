...
(our $VERSION) = sprintf "%03d", q$Revision: 1.5 $ =~ /Revision:\s+(\S+)/;
...
In CVS and RCS you use $Revision: 1.5 $ writing it like so:

    $VERSION = sprintf "%d.%03d", q$Revision: 1.5 $ =~ /(\d+)/g;

On your next check in, $Revision: 1.5 $ will magically be expanded to contain
the current revision #.

    $VERSION = sprintf "%d.%03d", q$Revision: 1.5 $ =~ /(\d+)/g;

Every time the file is checked in the $Revision: 1.5 $ will be updated,
...
1.10 to 1.010 which compares properly.
